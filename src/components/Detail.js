...
const Detail = ({match,dispatch,cart,count}) => {
  ...
  //选择尺码
  const handmashu =() => {
    dispatch(TodoActions.productaddMashuId(match.params.topicId))
  }
  const mashuId = match.params.topicId
  const id = match.params.topicId
  const product = count[id]
  const productMashuId = count[mashuId]
  var Addtoggle;
    ...
    <div className="detail fl">
      ...
      <div className="detail-content clear">
        ...
        <div className="fl content-right">
          ...
          <div className="mashu">
            码数:
            {productMashuId.mashu.map((num) =>
              <a onClick={ handmashu } className="num" key={num+'1'}>{num}</a>
            )}
          </div>
